...
public class BudgetActivity extends AppCompatActivity {
    ...
    private void loadWithLiveUsed(List<Budget> budgets) {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            for (Budget b : list) {
                ...
                double computed = db.transactionDao()
                        .getSumForCategoryBetween(b.category, monthStart, monthEnd);
                ...
                .setPositiveButton("Save", (d, w) -> {
                    ...
                    try {
                        ...
                        Executors.newSingleThreadExecutor().execute(() -> {
                            ...
                            Budget existing = db.budgetDao()
                                    .getByCategoryMonthYear(cat, month, year);

                            if (existing != null) {

                                existing.limitAmount = limit;

                                existing.usedAmount = db.transactionDao()
                                        .getSumForCategoryBetween(cat, monthStart, monthEnd);
                                ...
                                Budget b = new Budget(cat, limit, month, year,
                                        CategoryEngine.getInfo(cat).icon,
                                        CategoryEngine.getInfo(cat).color);
    ...
    private void showEditDialog(Budget budget) {
        ...
        for (int i = 0; i < cats.length; i++) {
            if (cats[i].equals(budget.category)) {
                sp.setSelection(i);
                break;
            }
        }
        ...
        ((LinearLayout) v).addView(tvInfo, 0);
    ...
    private void showAutoGenerateDialog() {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            runOnUiThread(() -> {
                ...
                sb.append("Based on your past ")
                        .append(suggestions.get(0).monthsAnalysed)
                ...
                for (AutoBudgetGenerator.BudgetSuggestion s : suggestions) {
                    ...
                    sb.append(s.category).append("\n")
                            .append("  Avg: ₹")
                            .append(String.format("%.0f", s.avgSpend))
                            .append("  →  Budget: ₹")
                            .append(String.format("%.0f", s.suggestedLimit))
                            .append("\n\n");
                }
                ...
                new AlertDialog.Builder(this, R.style.AlertDialogDark)
                        ...
                        .setPositiveButton("Apply All",
                                (d, w) -> applyAutoSuggestions(suggestions))
                        .setNegativeButton("Cancel", null)
                        .show();
            });
        });
    ...
    private void applyAutoSuggestions(
            List<AutoBudgetGenerator.BudgetSuggestion> suggestions) {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            for (AutoBudgetGenerator.BudgetSuggestion s : suggestions) {
                ...
                Budget existing = db.budgetDao()
                        .getByCategoryMonthYear(s.category, month, year);
                ...
                if (existing != null) {
                    ...
                    Budget b = new Budget(s.category, s.suggestedLimit,
                            month, year,
            ...
            runOnUiThread(() -> {
                ...
                Toast.makeText(this,
                        "✅ Applied " + count + " budget suggestion"
                                + (count == 1 ? "" : "s"),
